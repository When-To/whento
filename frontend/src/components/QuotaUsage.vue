...
<template>
  <div class="p-4 bg-white border border-gray-200 rounded-lg dark:bg-gray-900 dark:border-gray-800">
    <!-- Loading state -->
    <div v-if="loading" class="text-gray-500 text-sm dark:text-gray-400">
      {{ t('common.loading') }}
    </div>

    <!-- Error state -->
    <div v-else-if="error" class="text-red-600 text-sm dark:text-red-400">
      {{ error }}
    </div>

    <!-- Quota display -->
    <div v-else-if="quota" class="space-y-2">
      <!-- Header -->
      ...
      <div v-if="!isUnlimited" class="w-full bg-gray-200 rounded-full h-2 dark:bg-gray-700">
        <div
          :class="barColor"
